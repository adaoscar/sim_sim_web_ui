...
@Component({
  selector: 'sim-decisiontramite-table',
  templateUrl: './sim.decisiontramite.table.html',
  // styleUrls: ['./sim.decisiontramite.table.css'],
  providers: [SimDecisionTramiteService]
})
export class SimDecisionTramiteTable implements AfterViewInit {
    rows: SimDecisionTramiteModel[] = [];
    originals: SimDecisionTramiteModel[] = [];
    selectedRow: SimDecisionTramiteModel;
    selectedIndex: number = 0;
    originalRow: SimDecisionTramiteModel;

    public displayedColumns: string[] = ['simDecisionTramiteCodigo', 'simDecisionTramiteNombre', 'simDecisionTramiteEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simDecisionTramiteService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimDecisionTramiteModel();
      this.originalRow = new SimDecisionTramiteModel();
    ...
    onSelect(e: Event, row: SimDecisionTramiteModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimDecisionTramiteModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimDecisionTramiteModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimDecisionTramiteModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simDecisionTramiteService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimDecisionTramiteModel.cloneExcel(data.value), 'DecisionTramite');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simDecisionTramiteService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimDecisionTramiteModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimDecisionTramiteModel().fromClipboard(line);
            });

            this.simDecisionTramiteService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimDecisionTramite');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimDecisionTramiteDialog, {
          data: {
