...
@Component({
  selector: 'sim-escolaridad-table',
  templateUrl: './sim.escolaridad.table.html',
  // styleUrls: ['./sim.escolaridad.table.css'],
  providers: [SimEscolaridadService]
})
export class SimEscolaridadTable implements AfterViewInit {
    rows: SimEscolaridadModel[] = [];
    originals: SimEscolaridadModel[] = [];
    selectedRow: SimEscolaridadModel;
    selectedIndex: number = 0;
    originalRow: SimEscolaridadModel;

    public displayedColumns: string[] = ['simEscolaridadCodigo', 'simEscolaridadNombre', 'simEscolaridadEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simEscolaridadService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimEscolaridadModel();
      this.originalRow = new SimEscolaridadModel();
    ...
    onSelect(e: Event, row: SimEscolaridadModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimEscolaridadModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimEscolaridadModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimEscolaridadModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simEscolaridadService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimEscolaridadModel.cloneExcel(data.value), 'Escolaridad');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simEscolaridadService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimEscolaridadModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimEscolaridadModel().fromClipboard(line);
            });

            this.simEscolaridadService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimEscolaridad');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimEscolaridadDialog, {
          data: {
