...
export class SimTipoDecisionMockService {
    rows: Array<SimTipoDecisionModel> = [];
    autoincrement = this.rows.length;

    constructor() { }

    getById(simTipoDecisionId: number): Observable<any> {
        const _filtered = this.rows.filter((x) => x.SimTipoDecisionId === simTipoDecisionId);
    ...
    add(row: SimTipoDecisionModel): Observable<SimTipoDecisionModel> {
        let _row = SimTipoDecisionModel.clone(row);
        _row.SimTipoDecisionId = ++this.autoincrement;       
    ...
    update(row: SimTipoDecisionModel, original: SimTipoDecisionModel): Observable<SimTipoDecisionModel> {
        const inx = this.rows.findIndex((x) => x.SimTipoDecisionId === original.SimTipoDecisionId);

        let _row = null;

        if (inx >= 0) {
            this.rows[inx] = SimTipoDecisionModel.clone(row);
        } else {
            _row = {
                status: 404,
                statusText: "OK"
    ...
    delete(row: SimTipoDecisionModel): Observable<any> {
        const inx = this.rows.findIndex((x) => x.SimTipoDecisionId === row.SimTipoDecisionId);
