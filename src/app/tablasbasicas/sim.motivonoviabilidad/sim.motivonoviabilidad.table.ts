...
@Component({
  selector: 'sim-motivonoviabilidad-table',
  templateUrl: './sim.motivonoviabilidad.table.html',
  // styleUrls: ['./sim.motivonoviabilidad.table.css'],
  providers: [SimMotivoNoViabilidadService]
})
export class SimMotivoNoViabilidadTable implements AfterViewInit {
    rows: SimMotivoNoViabilidadModel[] = [];
    originals: SimMotivoNoViabilidadModel[] = [];
    selectedRow: SimMotivoNoViabilidadModel;
    selectedIndex: number = 0;
    originalRow: SimMotivoNoViabilidadModel;

    public displayedColumns: string[] = ['simMotivoNoViabilidadCodigo', 'simMotivoNoViabilidadNombre', 'simMotivoNoViabilidadEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simMotivoNoViabilidadService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimMotivoNoViabilidadModel();
      this.originalRow = new SimMotivoNoViabilidadModel();
    ...
    onSelect(e: Event, row: SimMotivoNoViabilidadModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimMotivoNoViabilidadModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimMotivoNoViabilidadModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimMotivoNoViabilidadModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simMotivoNoViabilidadService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimMotivoNoViabilidadModel.cloneExcel(data.value), 'MotivoNoViabilidad');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simMotivoNoViabilidadService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimMotivoNoViabilidadModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimMotivoNoViabilidadModel().fromClipboard(line);
            });

            this.simMotivoNoViabilidadService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimMotivoNoViabilidad');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimMotivoNoViabilidadDialog, {
          data: {
