...
@Component({
  selector: 'sim-motivosfinalizaciontramite-table',
  templateUrl: './sim.motivosfinalizaciontramite.table.html',
  // styleUrls: ['./sim.motivosfinalizaciontramite.table.css'],
  providers: [SimMotivosFinalizacionTramiteService]
})
export class SimMotivosFinalizacionTramiteTable implements AfterViewInit {
    rows: SimMotivosFinalizacionTramiteModel[] = [];
    originals: SimMotivosFinalizacionTramiteModel[] = [];
    selectedRow: SimMotivosFinalizacionTramiteModel;
    selectedIndex: number = 0;
    originalRow: SimMotivosFinalizacionTramiteModel;

    public displayedColumns: string[] = ['simMotivosFinalizacionTramiteCodigo', 'simMotivosFinalizacionTramiteNombre', 'simMotivosFinalizacionTramiteEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simMotivosFinalizacionTramiteService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimMotivosFinalizacionTramiteModel();
      this.originalRow = new SimMotivosFinalizacionTramiteModel();
    ...
    onSelect(e: Event, row: SimMotivosFinalizacionTramiteModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimMotivosFinalizacionTramiteModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimMotivosFinalizacionTramiteModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimMotivosFinalizacionTramiteModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simMotivosFinalizacionTramiteService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimMotivosFinalizacionTramiteModel.cloneExcel(data.value), 'MotivosFinalizacionTramite');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simMotivosFinalizacionTramiteService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimMotivosFinalizacionTramiteModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimMotivosFinalizacionTramiteModel().fromClipboard(line);
            });

            this.simMotivosFinalizacionTramiteService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimMotivosFinalizacionTramite');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimMotivosFinalizacionTramiteDialog, {
          data: {
