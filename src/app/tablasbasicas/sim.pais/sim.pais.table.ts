...
@Component({
  selector: 'sim-pais-table',
  templateUrl: './sim.pais.table.html',
  // styleUrls: ['./sim.pais.table.css'],
  providers: [SimPaisService]
})
export class SimPaisTable implements AfterViewInit {
    rows: SimPaisModel[] = [];
    originals: SimPaisModel[] = [];
    selectedRow: SimPaisModel;
    selectedIndex: number = 0;
    originalRow: SimPaisModel;

    public displayedColumns: string[] = ['simPaisCodigo', 'simPaisNombre', 'simPaisEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simPaisService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimPaisModel();
      this.originalRow = new SimPaisModel();
    ...
    onSelect(e: Event, row: SimPaisModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimPaisModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimPaisModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimPaisModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simPaisService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimPaisModel.cloneExcel(data.value), 'Pais');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simPaisService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimPaisModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimPaisModel().fromClipboard(line);
            });

            this.simPaisService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimPais');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimPaisDialog, {
          data: {
