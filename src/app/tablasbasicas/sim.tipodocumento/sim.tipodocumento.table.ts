...
@Component({
  selector: 'sim-tipodocumento-table',
  templateUrl: './sim.tipodocumento.table.html',
  // styleUrls: ['./sim.tipodocumento.table.css'],
  providers: [SimTipoDocumentoService]
})
export class SimTipoDocumentoTable implements AfterViewInit {
    rows: SimTipoDocumentoModel[] = [];
    originals: SimTipoDocumentoModel[] = [];
    selectedRow: SimTipoDocumentoModel;
    selectedIndex: number = 0;
    originalRow: SimTipoDocumentoModel;

    public displayedColumns: string[] = ['simTipoDocumentoCodigo', 'simTipoDocumentoNombre', 'simTipoDocumentoEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simTipoDocumentoService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimTipoDocumentoModel();
      this.originalRow = new SimTipoDocumentoModel();
    ...
    onSelect(e: Event, row: SimTipoDocumentoModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimTipoDocumentoModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimTipoDocumentoModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimTipoDocumentoModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simTipoDocumentoService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimTipoDocumentoModel.cloneExcel(data.value), 'TipoDocumento');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simTipoDocumentoService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimTipoDocumentoModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimTipoDocumentoModel().fromClipboard(line);
            });

            this.simTipoDocumentoService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimTipoDocumento');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimTipoDocumentoDialog, {
          data: {
