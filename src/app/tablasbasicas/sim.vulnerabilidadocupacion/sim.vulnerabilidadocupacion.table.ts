...
@Component({
  selector: 'sim-vulnerabilidadocupacion-table',
  templateUrl: './sim.vulnerabilidadocupacion.table.html',
  // styleUrls: ['./sim.vulnerabilidadocupacion.table.css'],
  providers: [SimVulnerabilidadOcupacionService]
})
export class SimVulnerabilidadOcupacionTable implements AfterViewInit {
    rows: SimVulnerabilidadOcupacionModel[] = [];
    originals: SimVulnerabilidadOcupacionModel[] = [];
    selectedRow: SimVulnerabilidadOcupacionModel;
    selectedIndex: number = 0;
    originalRow: SimVulnerabilidadOcupacionModel;

    public displayedColumns: string[] = ['simVulnerabilidadOcupacionCodigo', 'simVulnerabilidadOcupacionNombre', 'simVulnerabilidadOcupacionEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simVulnerabilidadOcupacionService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimVulnerabilidadOcupacionModel();
      this.originalRow = new SimVulnerabilidadOcupacionModel();
    ...
    onSelect(e: Event, row: SimVulnerabilidadOcupacionModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimVulnerabilidadOcupacionModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimVulnerabilidadOcupacionModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimVulnerabilidadOcupacionModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simVulnerabilidadOcupacionService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimVulnerabilidadOcupacionModel.cloneExcel(data.value), 'VulnerabilidadOcupacion');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simVulnerabilidadOcupacionService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimVulnerabilidadOcupacionModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimVulnerabilidadOcupacionModel().fromClipboard(line);
            });

            this.simVulnerabilidadOcupacionService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimVulnerabilidadOcupacion');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimVulnerabilidadOcupacionDialog, {
          data: {
