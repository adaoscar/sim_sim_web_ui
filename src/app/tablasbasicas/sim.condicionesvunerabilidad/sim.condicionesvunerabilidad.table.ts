...
@Component({
  selector: 'sim-condicionesvunerabilidad-table',
  templateUrl: './sim.condicionesvunerabilidad.table.html',
  // styleUrls: ['./sim.condicionesvunerabilidad.table.css'],
  providers: [SimCondicionesVunerabilidadService]
})
export class SimCondicionesVunerabilidadTable implements AfterViewInit {
    rows: SimCondicionesVunerabilidadModel[] = [];
    originals: SimCondicionesVunerabilidadModel[] = [];
    selectedRow: SimCondicionesVunerabilidadModel;
    selectedIndex: number = 0;
    originalRow: SimCondicionesVunerabilidadModel;

    public displayedColumns: string[] = ['simCondicionesVunerabilidadCodigo', 'simCondicionesVunerabilidadNombre', 'simCondicionesVunerabilidadEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simCondicionesVunerabilidadService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimCondicionesVunerabilidadModel();
      this.originalRow = new SimCondicionesVunerabilidadModel();
    ...
    onSelect(e: Event, row: SimCondicionesVunerabilidadModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimCondicionesVunerabilidadModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimCondicionesVunerabilidadModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimCondicionesVunerabilidadModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simCondicionesVunerabilidadService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimCondicionesVunerabilidadModel.cloneExcel(data.value), 'CondicionesVunerabilidad');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simCondicionesVunerabilidadService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimCondicionesVunerabilidadModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimCondicionesVunerabilidadModel().fromClipboard(line);
            });

            this.simCondicionesVunerabilidadService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimCondicionesVunerabilidad');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimCondicionesVunerabilidadDialog, {
          data: {
