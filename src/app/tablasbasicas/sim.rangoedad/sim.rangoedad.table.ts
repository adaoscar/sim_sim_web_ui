...
@Component({
  selector: 'sim-rangoedad-table',
  templateUrl: './sim.rangoedad.table.html',
  // styleUrls: ['./sim.rangoedad.table.css'],
  providers: [SimRangoEdadService]
})
export class SimRangoEdadTable implements AfterViewInit {
    rows: SimRangoEdadModel[] = [];
    originals: SimRangoEdadModel[] = [];
    selectedRow: SimRangoEdadModel;
    selectedIndex: number = 0;
    originalRow: SimRangoEdadModel;

    public displayedColumns: string[] = ['simRangoEdadCodigo', 'simRangoEdadNombre', 'simRangoEdadEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simRangoEdadService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimRangoEdadModel();
      this.originalRow = new SimRangoEdadModel();
    ...
    onSelect(e: Event, row: SimRangoEdadModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimRangoEdadModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimRangoEdadModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimRangoEdadModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simRangoEdadService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimRangoEdadModel.cloneExcel(data.value), 'RangoEdad');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simRangoEdadService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimRangoEdadModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimRangoEdadModel().fromClipboard(line);
            });

            this.simRangoEdadService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimRangoEdad');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimRangoEdadDialog, {
          data: {
