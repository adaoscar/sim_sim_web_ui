...
@Component({
  selector: 'sim-estadoasignacion-table',
  templateUrl: './sim.estadoasignacion.table.html',
  // styleUrls: ['./sim.estadoasignacion.table.css'],
  providers: [SimEstadoAsignacionService]
})
export class SimEstadoAsignacionTable implements AfterViewInit {
    rows: SimEstadoAsignacionModel[] = [];
    originals: SimEstadoAsignacionModel[] = [];
    selectedRow: SimEstadoAsignacionModel;
    selectedIndex: number = 0;
    originalRow: SimEstadoAsignacionModel;

    public displayedColumns: string[] = ['simEstadoAsignacionCodigo', 'simEstadoAsignacionNombre', 'simEstadoAsignacionEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simEstadoAsignacionService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimEstadoAsignacionModel();
      this.originalRow = new SimEstadoAsignacionModel();
    ...
    onSelect(e: Event, row: SimEstadoAsignacionModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimEstadoAsignacionModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimEstadoAsignacionModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimEstadoAsignacionModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simEstadoAsignacionService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimEstadoAsignacionModel.cloneExcel(data.value), 'EstadoAsignacion');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simEstadoAsignacionService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimEstadoAsignacionModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimEstadoAsignacionModel().fromClipboard(line);
            });

            this.simEstadoAsignacionService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimEstadoAsignacion');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimEstadoAsignacionDialog, {
          data: {
