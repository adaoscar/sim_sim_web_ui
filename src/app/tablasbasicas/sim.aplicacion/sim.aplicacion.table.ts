...
@Component({
  selector: 'sim-aplicacion-table',
  templateUrl: './sim.aplicacion.table.html',
  // styleUrls: ['./sim.aplicacion.table.css'],
  providers: [SimAplicacionService]
})
export class SimAplicacionTable implements AfterViewInit {
    rows: SimAplicacionModel[] = [];
    originals: SimAplicacionModel[] = [];
    selectedRow: SimAplicacionModel;
    selectedIndex: number = 0;
    originalRow: SimAplicacionModel;

    public displayedColumns: string[] = ['simAplicacionCodigo', 'simAplicacionNombre', 'simAplicacionEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simAplicacionService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimAplicacionModel();
      this.originalRow = new SimAplicacionModel();
    ...
    onSelect(e: Event, row: SimAplicacionModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimAplicacionModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimAplicacionModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimAplicacionModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simAplicacionService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimAplicacionModel.cloneExcel(data.value), 'Aplicacion');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simAplicacionService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimAplicacionModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimAplicacionModel().fromClipboard(line);
            });

            this.simAplicacionService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimAplicacion');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimAplicacionDialog, {
          data: {
