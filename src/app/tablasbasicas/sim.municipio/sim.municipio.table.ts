...
@Component({
  selector: 'sim-municipio-table',
  templateUrl: './sim.municipio.table.html',
  // styleUrls: ['./sim.municipio.table.css'],
  providers: [SimMunicipioService]
})
export class SimMunicipioTable implements AfterViewInit {
    rows: SimMunicipioModel[] = [];
    originals: SimMunicipioModel[] = [];
    selectedRow: SimMunicipioModel;
    selectedIndex: number = 0;
    originalRow: SimMunicipioModel;

    public displayedColumns: string[] = ['simDepartamento/simDepartamentoNombre', 'simMunicipioCodigo', 'siceqAutoridadJudicial/simMunicipioNombre', 'simMunicipioEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simMunicipioService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimMunicipioModel();
      this.originalRow = new SimMunicipioModel();
    ...
    onSelect(e: Event, row: SimMunicipioModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimMunicipioModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimMunicipioModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimMunicipioModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simMunicipioService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimMunicipioModel.cloneExcel(data.value), 'Municipio');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simMunicipioService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimMunicipioModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimMunicipioModel().fromClipboard(line);
            });

            this.simMunicipioService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimMunicipio');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimMunicipioDialog, {
          data: {
