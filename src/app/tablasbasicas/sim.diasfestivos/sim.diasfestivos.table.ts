...
@Component({
  selector: 'sim-diasfestivos-table',
  templateUrl: './sim.diasfestivos.table.html',
  // styleUrls: ['./sim.diasfestivos.table.css'],
  providers: [SimDiasFestivosService]
})
export class SimDiasFestivosTable implements AfterViewInit {
    rows: SimDiasFestivosModel[] = [];
    originals: SimDiasFestivosModel[] = [];
    selectedRow: SimDiasFestivosModel;
    selectedIndex: number = 0;
    originalRow: SimDiasFestivosModel;

    public displayedColumns: string[] = ['simDiasFestivosFecha'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simDiasFestivosService.getList(_filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimDiasFestivosModel();
      this.originalRow = new SimDiasFestivosModel();
    ...
    onSelect(e: Event, row: SimDiasFestivosModel, index: number) {
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimDiasFestivosModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimDiasFestivosModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimDiasFestivosModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simDiasFestivosService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimDiasFestivosModel.cloneExcel(data.value), 'DiasFestivos');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simDiasFestivosService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimDiasFestivosModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimDiasFestivosModel().fromClipboard(line);
            });

            this.simDiasFestivosService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimDiasFestivos');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimDiasFestivosDialog, {
          data: {
