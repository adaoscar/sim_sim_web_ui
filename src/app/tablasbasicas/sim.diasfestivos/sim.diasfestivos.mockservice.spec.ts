...
export class SimDiasFestivosMockService {
    rows: Array<SimDiasFestivosModel> = [];
    autoincrement = this.rows.length;

    constructor() { }

    getById(simDiasFestivosId: number): Observable<any> {
        const _filtered = this.rows.filter((x) => x.SimDiasFestivosId === simDiasFestivosId);
    ...
    add(row: SimDiasFestivosModel): Observable<SimDiasFestivosModel> {
        let _row = SimDiasFestivosModel.clone(row);
        _row.SimDiasFestivosId = ++this.autoincrement;       
    ...
    update(row: SimDiasFestivosModel, original: SimDiasFestivosModel): Observable<SimDiasFestivosModel> {
        const inx = this.rows.findIndex((x) => x.SimDiasFestivosId === original.SimDiasFestivosId);

        let _row = null;

        if (inx >= 0) {
            this.rows[inx] = SimDiasFestivosModel.clone(row);
        } else {
            _row = {
                status: 404,
                statusText: "OK"
    ...
    delete(row: SimDiasFestivosModel): Observable<any> {
        const inx = this.rows.findIndex((x) => x.SimDiasFestivosId === row.SimDiasFestivosId);
