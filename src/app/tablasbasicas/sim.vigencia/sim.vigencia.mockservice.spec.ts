...
export class SimVigenciaMockService {
    rows: Array<SimVigenciaModel> = [];
    autoincrement = this.rows.length;

    constructor() { }

    getById(simVigenciaId: number): Observable<any> {
        const _filtered = this.rows.filter((x) => x.SimVigenciaId === simVigenciaId);
    ...
    add(row: SimVigenciaModel): Observable<SimVigenciaModel> {
        let _row = SimVigenciaModel.clone(row);
        _row.SimVigenciaId = ++this.autoincrement;       
    ...
    update(row: SimVigenciaModel, original: SimVigenciaModel): Observable<SimVigenciaModel> {
        const inx = this.rows.findIndex((x) => x.SimVigenciaId === original.SimVigenciaId);

        let _row = null;

        if (inx >= 0) {
            this.rows[inx] = SimVigenciaModel.clone(row);
        } else {
            _row = {
                status: 404,
                statusText: "OK"
    ...
    delete(row: SimVigenciaModel): Observable<any> {
        const inx = this.rows.findIndex((x) => x.SimVigenciaId === row.SimVigenciaId);
