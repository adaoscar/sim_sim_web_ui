...
@Component({
  selector: 'sim-vigencia-table',
  templateUrl: './sim.vigencia.table.html',
  // styleUrls: ['./sim.vigencia.table.css'],
  providers: [SimVigenciaService]
})
export class SimVigenciaTable implements AfterViewInit {
    rows: SimVigenciaModel[] = [];
    originals: SimVigenciaModel[] = [];
    selectedRow: SimVigenciaModel;
    selectedIndex: number = 0;
    originalRow: SimVigenciaModel;

    public displayedColumns: string[] = ['simVigenciaCodigo', 'simVigenciaNombre', 'simVigenciaEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simVigenciaService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimVigenciaModel();
      this.originalRow = new SimVigenciaModel();
    ...
    onSelect(e: Event, row: SimVigenciaModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimVigenciaModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimVigenciaModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimVigenciaModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simVigenciaService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimVigenciaModel.cloneExcel(data.value), 'Vigencia');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simVigenciaService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimVigenciaModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimVigenciaModel().fromClipboard(line);
            });

            this.simVigenciaService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimVigencia');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimVigenciaDialog, {
          data: {
