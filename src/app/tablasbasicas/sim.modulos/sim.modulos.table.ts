...
@Component({
  selector: 'sim-modulos-table',
  templateUrl: './sim.modulos.table.html',
  // styleUrls: ['./sim.modulos.table.css'],
  providers: [SimModulosService]
})
export class SimModulosTable implements AfterViewInit {
    rows: SimModulosModel[] = [];
    originals: SimModulosModel[] = [];
    selectedRow: SimModulosModel;
    selectedIndex: number = 0;
    originalRow: SimModulosModel;

    public displayedColumns: string[] = ['simModulosCodigo', 'simModulosNombre', 'simModulosEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simModulosService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimModulosModel();
      this.originalRow = new SimModulosModel();
    ...
    onSelect(e: Event, row: SimModulosModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimModulosModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimModulosModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimModulosModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simModulosService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimModulosModel.cloneExcel(data.value), 'Modulos');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simModulosService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimModulosModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimModulosModel().fromClipboard(line);
            });

            this.simModulosService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimModulos');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimModulosDialog, {
          data: {
