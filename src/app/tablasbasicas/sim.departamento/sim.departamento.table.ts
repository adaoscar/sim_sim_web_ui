...
@Component({
  selector: 'sim-departamento-table',
  templateUrl: './sim.departamento.table.html',
  // styleUrls: ['./sim.departamento.table.css'],
  providers: [SimDepartamentoService]
})
export class SimDepartamentoTable implements AfterViewInit {
    rows: SimDepartamentoModel[] = [];
    originals: SimDepartamentoModel[] = [];
    selectedRow: SimDepartamentoModel;
    selectedIndex: number = 0;
    originalRow: SimDepartamentoModel;

    public displayedColumns: string[] = ['simDepartamentoCodigo', 'simDepartamentoNombre', 'simDepartamentoEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simDepartamentoService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimDepartamentoModel();
      this.originalRow = new SimDepartamentoModel();
    ...
    onSelect(e: Event, row: SimDepartamentoModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimDepartamentoModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimDepartamentoModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimDepartamentoModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simDepartamentoService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimDepartamentoModel.cloneExcel(data.value), 'Departamento');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simDepartamentoService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimDepartamentoModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimDepartamentoModel().fromClipboard(line);
            });

            this.simDepartamentoService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimDepartamento');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimDepartamentoDialog, {
          data: {
