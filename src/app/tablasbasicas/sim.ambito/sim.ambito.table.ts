...
@Component({
  selector: 'sim-ambito-table',
  templateUrl: './sim.ambito.table.html',
  // styleUrls: ['./sim.ambito.table.css'],
  providers: [SimAmbitoService]
})
export class SimAmbitoTable implements AfterViewInit {
    rows: SimAmbitoModel[] = [];
    originals: SimAmbitoModel[] = [];
    selectedRow: SimAmbitoModel;
    selectedIndex: number = 0;
    originalRow: SimAmbitoModel;

    public displayedColumns: string[] = ['simAmbitoCodigo', 'simAmbitoNombre', 'simAmbitoEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simAmbitoService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimAmbitoModel();
      this.originalRow = new SimAmbitoModel();
    ...
    onSelect(e: Event, row: SimAmbitoModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimAmbitoModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimAmbitoModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimAmbitoModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simAmbitoService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimAmbitoModel.cloneExcel(data.value), 'Ambito');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simAmbitoService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimAmbitoModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimAmbitoModel().fromClipboard(line);
            });

            this.simAmbitoService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimAmbito');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimAmbitoDialog, {
          data: {
