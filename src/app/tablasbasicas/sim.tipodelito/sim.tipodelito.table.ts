...
@Component({
  selector: 'sim-tipodelito-table',
  templateUrl: './sim.tipodelito.table.html',
  // styleUrls: ['./sim.tipodelito.table.css'],
  providers: [SimTipoDelitoService]
})
export class SimTipoDelitoTable implements AfterViewInit {
    rows: SimTipoDelitoModel[] = [];
    originals: SimTipoDelitoModel[] = [];
    selectedRow: SimTipoDelitoModel;
    selectedIndex: number = 0;
    originalRow: SimTipoDelitoModel;

    public displayedColumns: string[] = ['simTipoDelitoCodigo', 'simTipoDelitoNombre', 'simTipoDelitoEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simTipoDelitoService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimTipoDelitoModel();
      this.originalRow = new SimTipoDelitoModel();
    ...
    onSelect(e: Event, row: SimTipoDelitoModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimTipoDelitoModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimTipoDelitoModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimTipoDelitoModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simTipoDelitoService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimTipoDelitoModel.cloneExcel(data.value), 'TipoDelito');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simTipoDelitoService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimTipoDelitoModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimTipoDelitoModel().fromClipboard(line);
            });

            this.simTipoDelitoService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimTipoDelito');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimTipoDelitoDialog, {
          data: {
