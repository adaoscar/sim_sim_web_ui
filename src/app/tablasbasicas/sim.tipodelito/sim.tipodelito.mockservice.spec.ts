...
export class SimTipoDelitoMockService {
    rows: Array<SimTipoDelitoModel> = [];
    autoincrement = this.rows.length;

    constructor() { }

    getById(simTipoDelitoId: number): Observable<any> {
        const _filtered = this.rows.filter((x) => x.SimTipoDelitoId === simTipoDelitoId);
    ...
    add(row: SimTipoDelitoModel): Observable<SimTipoDelitoModel> {
        let _row = SimTipoDelitoModel.clone(row);
        _row.SimTipoDelitoId = ++this.autoincrement;       
    ...
    update(row: SimTipoDelitoModel, original: SimTipoDelitoModel): Observable<SimTipoDelitoModel> {
        const inx = this.rows.findIndex((x) => x.SimTipoDelitoId === original.SimTipoDelitoId);

        let _row = null;

        if (inx >= 0) {
            this.rows[inx] = SimTipoDelitoModel.clone(row);
        } else {
            _row = {
                status: 404,
                statusText: "OK"
    ...
    delete(row: SimTipoDelitoModel): Observable<any> {
        const inx = this.rows.findIndex((x) => x.SimTipoDelitoId === row.SimTipoDelitoId);
