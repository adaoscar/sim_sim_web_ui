...
@Component({
  selector: 'sim-periododias-table',
  templateUrl: './sim.periododias.table.html',
  // styleUrls: ['./sim.periododias.table.css'],
  providers: [SimPeriodoDiasService]
})
export class SimPeriodoDiasTable implements AfterViewInit {
    rows: SimPeriodoDiasModel[] = [];
    originals: SimPeriodoDiasModel[] = [];
    selectedRow: SimPeriodoDiasModel;
    selectedIndex: number = 0;
    originalRow: SimPeriodoDiasModel;

    public displayedColumns: string[] = ['simPeriodoDiasCodigo', 'simPeriodoDiasNombre', 'simPeriodoDiasEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simPeriodoDiasService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimPeriodoDiasModel();
      this.originalRow = new SimPeriodoDiasModel();
    ...
    onSelect(e: Event, row: SimPeriodoDiasModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimPeriodoDiasModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimPeriodoDiasModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimPeriodoDiasModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simPeriodoDiasService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimPeriodoDiasModel.cloneExcel(data.value), 'PeriodoDias');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simPeriodoDiasService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimPeriodoDiasModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimPeriodoDiasModel().fromClipboard(line);
            });

            this.simPeriodoDiasService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimPeriodoDias');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimPeriodoDiasDialog, {
          data: {
