// SimMedioComunicacion - Service - Angular Testing
...
describe('SimMedioComunicacionService', () => {
    let httpClientSpy: {
      get: jasmine.Spy,
      post: jasmine.Spy,
      put: jasmine.Spy,
    ...
    beforeEach(() => {
        ...
        service = new SimMedioComunicacionService(httpClientSpy as any);
    });

    it('#getById should return One Row', (done: DoneFn) => {
        httpClientSpy.get.and.returnValue(asyncData(rowBase));

        let row = new SimMedioComunicacionModel(rowBase);

        service.getById(row.SimMedioComunicacionId).subscribe((value) => {
			    expect(value.SimMedioComunicacionId).toBe(row.SimMedioComunicacionId);
			    expect(value.SimMedioComunicacionCodigo).toBe(row.SimMedioComunicacionCodigo);
			    expect(value.SimMedioComunicacionNombre).toBe(row.SimMedioComunicacionNombre);
			    expect(value.SimMedioComunicacionEstado).toBe(row.SimMedioComunicacionEstado);
          done();
        });
    });
    ...
    it('#getById should return 404 Not found', (done: DoneFn) => {
        ...
        service.getById(row.SimMedioComunicacionId).subscribe(
            (value) => {
            ...
            (error) => {
                fail(`expected an error, not SimMedioComunicacionModel ${JSON.stringify(error)}`);
                done();
            }
        );
    });
    ...
    it('#save-Add should return a Add row', (done: DoneFn) => {
        ...
        let row = new SimMedioComunicacionModel(rowBase);
        row._estado = 'N';
        delete row.SimMedioComunicacionId;

        //Add - SimMedioComunicacion
    ...
    it('#save-update should return 404 Not found', (done: DoneFn) => {
        ...
        let row = new SimMedioComunicacionModel({});
        row.SimMedioComunicacionId = -1;
        ...
        service.save(row, row).subscribe(
            ...
            (error) => {
                fail(`expected an error for update, not SimMedioComunicacionModel ${JSON.stringify(error)}`);
                done();
            }
        );
    });
