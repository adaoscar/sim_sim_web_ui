...
@Component({
  selector: 'sim-mediocomunicacion-table',
  templateUrl: './sim.mediocomunicacion.table.html',
  // styleUrls: ['./sim.mediocomunicacion.table.css'],
  providers: [SimMedioComunicacionService]
})
export class SimMedioComunicacionTable implements AfterViewInit {
    rows: SimMedioComunicacionModel[] = [];
    originals: SimMedioComunicacionModel[] = [];
    selectedRow: SimMedioComunicacionModel;
    selectedIndex: number = 0;
    originalRow: SimMedioComunicacionModel;

    public displayedColumns: string[] = ['simMedioComunicacionCodigo', 'simMedioComunicacionNombre', 'simMedioComunicacionEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simMedioComunicacionService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimMedioComunicacionModel();
      this.originalRow = new SimMedioComunicacionModel();
    ...
    onSelect(e: Event, row: SimMedioComunicacionModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimMedioComunicacionModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimMedioComunicacionModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimMedioComunicacionModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simMedioComunicacionService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimMedioComunicacionModel.cloneExcel(data.value), 'MedioComunicacion');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simMedioComunicacionService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimMedioComunicacionModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimMedioComunicacionModel().fromClipboard(line);
            });

            this.simMedioComunicacionService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimMedioComunicacion');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimMedioComunicacionDialog, {
          data: {
