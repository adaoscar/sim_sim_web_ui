...
export class SimMedioComunicacionMockService {
    rows: Array<SimMedioComunicacionModel> = [];
    autoincrement = this.rows.length;

    constructor() { }

    getById(simMedioComunicacionId: number): Observable<any> {
        const _filtered = this.rows.filter((x) => x.SimMedioComunicacionId === simMedioComunicacionId);
    ...
    add(row: SimMedioComunicacionModel): Observable<SimMedioComunicacionModel> {
        let _row = SimMedioComunicacionModel.clone(row);
        _row.SimMedioComunicacionId = ++this.autoincrement;       
    ...
    update(row: SimMedioComunicacionModel, original: SimMedioComunicacionModel): Observable<SimMedioComunicacionModel> {
        const inx = this.rows.findIndex((x) => x.SimMedioComunicacionId === original.SimMedioComunicacionId);

        let _row = null;

        if (inx >= 0) {
            this.rows[inx] = SimMedioComunicacionModel.clone(row);
        } else {
            _row = {
                status: 404,
                statusText: "OK"
    ...
    delete(row: SimMedioComunicacionModel): Observable<any> {
        const inx = this.rows.findIndex((x) => x.SimMedioComunicacionId === row.SimMedioComunicacionId);
