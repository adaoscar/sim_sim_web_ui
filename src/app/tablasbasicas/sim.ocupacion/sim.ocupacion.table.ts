...
@Component({
  selector: 'sim-ocupacion-table',
  templateUrl: './sim.ocupacion.table.html',
  // styleUrls: ['./sim.ocupacion.table.css'],
  providers: [SimOcupacionService]
})
export class SimOcupacionTable implements AfterViewInit {
    rows: SimOcupacionModel[] = [];
    originals: SimOcupacionModel[] = [];
    selectedRow: SimOcupacionModel;
    selectedIndex: number = 0;
    originalRow: SimOcupacionModel;

    public displayedColumns: string[] = ['simOcupacionCodigo', 'simOcupacionNombre', 'simOcupacionEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simOcupacionService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimOcupacionModel();
      this.originalRow = new SimOcupacionModel();
    ...
    onSelect(e: Event, row: SimOcupacionModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimOcupacionModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimOcupacionModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimOcupacionModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simOcupacionService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimOcupacionModel.cloneExcel(data.value), 'Ocupacion');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simOcupacionService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimOcupacionModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimOcupacionModel().fromClipboard(line);
            });

            this.simOcupacionService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimOcupacion');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimOcupacionDialog, {
          data: {
