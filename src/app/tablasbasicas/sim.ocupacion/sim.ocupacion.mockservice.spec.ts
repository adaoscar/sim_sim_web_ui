...
export class SimOcupacionMockService {
    rows: Array<SimOcupacionModel> = [];
    autoincrement = this.rows.length;

    constructor() { }

    getById(simOcupacionId: number): Observable<any> {
        const _filtered = this.rows.filter((x) => x.SimOcupacionId === simOcupacionId);
    ...
    add(row: SimOcupacionModel): Observable<SimOcupacionModel> {
        let _row = SimOcupacionModel.clone(row);
        _row.SimOcupacionId = ++this.autoincrement;       
    ...
    update(row: SimOcupacionModel, original: SimOcupacionModel): Observable<SimOcupacionModel> {
        const inx = this.rows.findIndex((x) => x.SimOcupacionId === original.SimOcupacionId);

        let _row = null;

        if (inx >= 0) {
            this.rows[inx] = SimOcupacionModel.clone(row);
        } else {
            _row = {
                status: 404,
                statusText: "OK"
    ...
    delete(row: SimOcupacionModel): Observable<any> {
        const inx = this.rows.findIndex((x) => x.SimOcupacionId === row.SimOcupacionId);
