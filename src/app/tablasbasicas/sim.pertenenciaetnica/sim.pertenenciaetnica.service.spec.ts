// SimPertenenciaEtnica - Service - Angular Testing
...
describe('SimPertenenciaEtnicaService', () => {
    let httpClientSpy: {
      get: jasmine.Spy,
      post: jasmine.Spy,
      put: jasmine.Spy,
    ...
    beforeEach(() => {
        ...
        service = new SimPertenenciaEtnicaService(httpClientSpy as any);
    });

    it('#getById should return One Row', (done: DoneFn) => {
        httpClientSpy.get.and.returnValue(asyncData(rowBase));

        let row = new SimPertenenciaEtnicaModel(rowBase);

        service.getById(row.SimPertenenciaEtnicaId).subscribe((value) => {
			    expect(value.SimPertenenciaEtnicaId).toBe(row.SimPertenenciaEtnicaId);
			    expect(value.SimPertenenciaEtnicaCodigo).toBe(row.SimPertenenciaEtnicaCodigo);
			    expect(value.SimPertenenciaEtnicaNombre).toBe(row.SimPertenenciaEtnicaNombre);
			    expect(value.SimPertenenciaEtnicaEstado).toBe(row.SimPertenenciaEtnicaEstado);
          done();
        });
    });
    ...
    it('#getById should return 404 Not found', (done: DoneFn) => {
        ...
        service.getById(row.SimPertenenciaEtnicaId).subscribe(
            (value) => {
            ...
            (error) => {
                fail(`expected an error, not SimPertenenciaEtnicaModel ${JSON.stringify(error)}`);
                done();
            }
        );
    });
    ...
    it('#save-Add should return a Add row', (done: DoneFn) => {
        ...
        let row = new SimPertenenciaEtnicaModel(rowBase);
        row._estado = 'N';
        delete row.SimPertenenciaEtnicaId;

        //Add - SimPertenenciaEtnica
    ...
    it('#save-update should return 404 Not found', (done: DoneFn) => {
        ...
        let row = new SimPertenenciaEtnicaModel({});
        row.SimPertenenciaEtnicaId = -1;
        ...
        service.save(row, row).subscribe(
            ...
            (error) => {
                fail(`expected an error for update, not SimPertenenciaEtnicaModel ${JSON.stringify(error)}`);
                done();
            }
        );
    });
