...
@Component({
  selector: 'sim-pertenenciaetnica-table',
  templateUrl: './sim.pertenenciaetnica.table.html',
  // styleUrls: ['./sim.pertenenciaetnica.table.css'],
  providers: [SimPertenenciaEtnicaService]
})
export class SimPertenenciaEtnicaTable implements AfterViewInit {
    rows: SimPertenenciaEtnicaModel[] = [];
    originals: SimPertenenciaEtnicaModel[] = [];
    selectedRow: SimPertenenciaEtnicaModel;
    selectedIndex: number = 0;
    originalRow: SimPertenenciaEtnicaModel;

    public displayedColumns: string[] = ['simPertenenciaEtnicaCodigo', 'simPertenenciaEtnicaNombre', 'simPertenenciaEtnicaEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simPertenenciaEtnicaService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimPertenenciaEtnicaModel();
      this.originalRow = new SimPertenenciaEtnicaModel();
    ...
    onSelect(e: Event, row: SimPertenenciaEtnicaModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimPertenenciaEtnicaModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimPertenenciaEtnicaModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimPertenenciaEtnicaModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simPertenenciaEtnicaService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimPertenenciaEtnicaModel.cloneExcel(data.value), 'PertenenciaEtnica');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simPertenenciaEtnicaService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimPertenenciaEtnicaModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimPertenenciaEtnicaModel().fromClipboard(line);
            });

            this.simPertenenciaEtnicaService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimPertenenciaEtnica');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimPertenenciaEtnicaDialog, {
          data: {
