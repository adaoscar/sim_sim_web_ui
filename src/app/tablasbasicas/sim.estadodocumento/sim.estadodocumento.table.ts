...
@Component({
  selector: 'sim-estadodocumento-table',
  templateUrl: './sim.estadodocumento.table.html',
  // styleUrls: ['./sim.estadodocumento.table.css'],
  providers: [SimEstadoDocumentoService]
})
export class SimEstadoDocumentoTable implements AfterViewInit {
    rows: SimEstadoDocumentoModel[] = [];
    originals: SimEstadoDocumentoModel[] = [];
    selectedRow: SimEstadoDocumentoModel;
    selectedIndex: number = 0;
    originalRow: SimEstadoDocumentoModel;

    public displayedColumns: string[] = ['simEstadoDocumentoCodigo', 'simEstadoDocumentoNombre', 'simEstadoDocumentoEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simEstadoDocumentoService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimEstadoDocumentoModel();
      this.originalRow = new SimEstadoDocumentoModel();
    ...
    onSelect(e: Event, row: SimEstadoDocumentoModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimEstadoDocumentoModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimEstadoDocumentoModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimEstadoDocumentoModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simEstadoDocumentoService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimEstadoDocumentoModel.cloneExcel(data.value), 'EstadoDocumento');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simEstadoDocumentoService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimEstadoDocumentoModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimEstadoDocumentoModel().fromClipboard(line);
            });

            this.simEstadoDocumentoService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimEstadoDocumento');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimEstadoDocumentoDialog, {
          data: {
