...
@Component({
  selector: 'sim-sexo-table',
  templateUrl: './sim.sexo.table.html',
  // styleUrls: ['./sim.sexo.table.css'],
  providers: [SimSexoService]
})
export class SimSexoTable implements AfterViewInit {
    rows: SimSexoModel[] = [];
    originals: SimSexoModel[] = [];
    selectedRow: SimSexoModel;
    selectedIndex: number = 0;
    originalRow: SimSexoModel;

    public displayedColumns: string[] = ['simSexoCodigo', 'simSexoNombre', 'simSexoEstado'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.simSexoService.getList(this.filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new SimSexoModel();
      this.originalRow = new SimSexoModel();
    ...
    onSelect(e: Event, row: SimSexoModel, index: number) {
        this._expand = this.selectedRow !== row ? true : !this._expand;
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = SimSexoModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: SimSexoModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = SimSexoModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.simSexoService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(SimSexoModel.cloneExcel(data.value), 'Sexo');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.simSexoService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new SimSexoModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new SimSexoModel().fromClipboard(line);
            });

            this.simSexoService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for SimSexo');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(SimSexoDialog, {
          data: {
